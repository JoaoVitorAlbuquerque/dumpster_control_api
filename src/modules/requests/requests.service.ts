...
export class RequestsService {
  ...
  async listApprovedRequests(ids?: string[]) {
    return this.requestsRepo.findMany({
      where: {
        status: 'APPROVED',
        isActive: true,
  ...
  // Novo método para buscar por IDs
  async findManyByIds(accountId: string, ids: string[]) {
    await this.validateUserRoleService.validate(accountId);
  ...
  // Extraia a lógica de geração de PDF para um método reutilizável
  async generatePdf(requests: any[], res: Response) {
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename=relatorio.pdf');

    // ...todo o código PDFDocument que você já tem no controller,
  ...
  */
    ...
    doc
      ...
      .fontSize(10)
      .fillColor('#111827')
      .text(`Total de solicitações aprovadas: ${requests.length}`, 40, y);
